...
from homeassistant.components.binary_sensor import BinarySensorDevice, ENTITY_ID_FORMAT, PLATFORM_SCHEMA
from homeassistant.components.device_tracker import ATTR_SOURCE_TYPE
from homeassistant.const import (ATTR_FRIENDLY_NAME, CONF_ENTITIES, EVENT_HOMEASSISTANT_START)
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({ vol.Required(CONF_ENTITIES): cv.entity_ids })
...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    for device in config[CONF_ENTITIES]:
        ...
        sensors.append(DeviceTrackerSensor(hass, "device_tracker_{0}".format(device.split(".", 1)[1]), friendly_name, source_type, state_template, device))
...
class DeviceTrackerSensor(BinarySensorDevice):
    ...
    def __init__(self, hass, device_id, friendly_name, source_type, state_template, entity_id):
    ...
    def async_added_to_hass(self):
        ...
        def template_sensor_startup(event):
            """Update on startup."""
            async_track_state_change(self.hass, self._entity, template_sensor_state_listener)
            self.hass.async_add_job(self.async_update_ha_state(True))

        self.hass.bus.async_listen_once(EVENT_HOMEASSISTANT_START, template_sensor_startup)
    ...
    def async_update(self):
        ...
        if entity_state is not None:
            device_friendly_name = entity_state.attributes.get(ATTR_FRIENDLY_NAME)
        ...
        try:
            ...
            if ex.args and ex.args[0].startswith("UndefinedError: 'None' has no attribute"):
                ...
                _LOGGER.warning('Could not render attribute sensor for %s, the state is unknown.', self._entity)
                return
            self._state = False
